...
\begin{frame}
	...
	\begin{itemize}[<+->]
		\item   Für $\overline{SAT}$, Sprache der nicht erfüllbaren Formeln, gilt $\overline{SAT} \in {\P}^{SAT}$.
		\item   Mit  Orakel $SAT$ kann TM in $\mathcal{O}(1)$ entscheiden, ob $\varphi \in SAT$ und gegenteilige Antwort ausgeben.
	\end{itemize}
	\bigskip
	\pause
	\heading{$EXPCOM$} 
	\begin{itemize} [<+->]
		\item Sei $EXPCOM$ folgende Sprache: \[\{(M,x,1^n) : M \ berechnet \ 1 \ bei \ Eingabe \ x \ innerhalb \ von \ 2^n \ Schritten  \}
		 \] 
			Dann gilt ${\P}^{EXPCOM} = {\NP}^{EXPCOM} = \EXP$.
			\item Wegen Orakel aus $\EXP$  $ \Rightarrow \EXP \subseteq {\P}^{EXPCOM} $
			\item Außerdem: $M$ eine nichtdet. TM mit Orakel $EXPCOM$: 
				\begin{itemize}[<+->]
				...
				\end{itemize}
			\item $\Rightarrow \EXP \subseteq{\P}^{EXPCOM} \subseteq {\NP}^{EXPCOM} \subseteq \EXP$
